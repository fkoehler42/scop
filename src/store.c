...
/*   Updated: 2018/06/15 11:46:06 by fkoehler         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	store_face(char **data, unsigned int **array, unsigned int *nb_face)
{
	...
	if (--i == 4)
	{
		array[f_idx][0] = strtoul(data[4]) - 1;
		array[f_idx][1] = strtoul(data[1]) - 1;
		array[f_idx][2] = strtoul(data[2]) - 1;
		f_idx++;
		store_face(&(data[1]), array, nb_face);
		return ;
	}
	while (--i >= 0)
		array[f_idx][i] = strtoul(data[i + 1]) - 1;
